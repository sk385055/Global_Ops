...
#JOSE CODED drop zero Qty
data_raw = data_raw[(data_raw['Plan Quantity'] != 0)]
...
#Jose Country Code details
# 1. Project name, 2. Order number
...
#filter only for plan IR/IO, JOSE added OH for CC-----------------------------------------------
...
#----------------------------------------------------------------------------------------------
#country code save in csv only for PBO and sales orders, JOSE ADDED EX type for CC 19-FEB-2025
...
#--------------------------------------------------------
#updating country code once again 'JOSE update both country code and customer
...
#item type
data['Item Type'] = ''
cond_k = data['Inventory Item Number Hyphenated'].str.match('^[0-9]{4}-K[A-Z0-9]{3}-V[0-9]{3}') #jose edited
...
data['Suggested Due Date'] = pd.to_datetime(data['Suggested Due Date'],format='%d-%m-%Y').dt.date
...
#------------------------------------------------------------------------------------------------------------------------------
#create multiple dataframe for updatting Supplier ' JOSE
excess_cntry = data_bk.copy()
data_item = data_bk.copy()
data_po = data_bk.copy()
data_make = data_bk.copy()
data_int_req = data_bk.copy()
data_plan_io = data_bk.copy()   #jose added
data_ex = data_bk.copy()   #jose added
data_int_ord = data_bk.copy()   #jose added
...
#filter for firmed excess data 'JOSE for Unknown Supplier
...
#Plan Make Sour --- jose comments to aviod duplicate in concate Ex
...
plant = ['EBS:UF2','EBS:GF1','EBS:USL','EBS:US']
...
#JOSE added Int Ord on 26-FEB-2025
#Internal Orders
data_int_ord= data_int_ord[['Inventory Item Number Hyphenated','Source Supplier Site Code','Order Type','Source Organization Code']]
data_int_ord = data_int_ord[data_int_ord['Order Type'].isin(['Int Order'])]
plant = ['EBS:UF2','EBS:GF1','EBS:USL','EBS:US']
data_int_ord =data_int_ord[data_int_ord['Source Organization Code'].isin(plant)]

data_int_ord['Plant_new'] =''
data_int_ord['Splr_new'] =''
for i in range(len(plant)):
    data_int_ord['Plant_new'] = np.where(data_int_ord['Source Organization Code'].isin([plant[i]]) & data_int_ord['Plant_new'].isin(['']) ,plant[i][4:7],data_int_ord['Plant_new'])
    data_int_ord['Splr_new'] = np.where(data_int_ord['Source Organization Code'].isin([plant[i]]) & data_int_ord['Splr_new'].isin(['']),plant[i],data_int_ord['Splr_new'])
    
data_int_ord.drop_duplicates(inplace=True,subset=['Inventory Item Number Hyphenated'],keep='first')
data_int_ord['Data_ref'] = 'Int Order'


#jose plan IRIO
#Plan IR/IO
data_plan_io= data_plan_io[['Inventory Item Number Hyphenated','Source Supplier Site Code','Order Type','Source Organization Code']]
data_plan_io = data_plan_io[data_plan_io['Order Type'].isin(['Plan IR/IO'])]
plant = ['EBS:GF1','EBS:USL','EBS:COL','EBS:UF2']
data_plan_io =data_plan_io[data_plan_io['Source Organization Code'].isin(plant)]
data_plan_io['Plant_new'] =''
data_plan_io['Splr_new'] =''
for i in range(len(plant)):
    data_plan_io['Plant_new'] = np.where(data_plan_io['Source Organization Code'].isin([plant[i]]) & data_plan_io['Plant_new'].isin(['']) ,plant[i][4:7],data_plan_io['Plant_new'])
    data_plan_io['Splr_new'] = np.where(data_plan_io['Source Organization Code'].isin([plant[i]]) & data_plan_io['Splr_new'].isin(['']),plant[i],data_plan_io['Splr_new'])

#data_plan_io.drop_duplicates(inplace=True,subset=['Inventory Item Number Hyphenated'],keep='first')
data_plan_io['Data_ref'] = 'Plan IRIO'

#EX jose added on 18-Apr-2024
data_ex= data_ex[['Inventory Item Number Hyphenated','Source Supplier Site Code','Order Type','Action Description']]
data_ex = data_ex[data_ex['Order Type'].isin(['EX'])]
splr = ['EDI-BUDAPES-USD','EDI-CHI-201-USD','P6DEL-GUAD','P6-HOL-201-USD']
data_ex =data_ex[data_ex['Source Supplier Site Code'].isin(splr)]

plant_new=['EC','JAB','USL','USL']
data_ex['Plant_new'] =''
data_ex['Splr_new'] =''
for i in range(len(splr)):
    data_ex['Plant_new'] = np.where(data_ex['Source Supplier Site Code'].isin([splr[i]]) & data_ex['Plant_new'].isin(['']) ,plant_new[i],data_ex['Plant_new'])
    data_ex['Splr_new'] = np.where(data_ex['Source Supplier Site Code'].isin([splr[i]]) & data_ex['Splr_new'].isin(['']),splr[i],data_ex['Splr_new'])

data_ex.sort_values(by=['Inventory Item Number Hyphenated', 'Action Description'], ascending=[True, False],inplace = True)
data_ex.drop_duplicates(inplace=True,subset=['Inventory Item Number Hyphenated'],keep='first')
data_ex['Data_ref'] = 'EX'

#concat all
# data_con = pd.concat([data_po,data_make,data_int_req,data_plan_io,data_ex],axis=0)
data_con = pd.concat([data_ex,data_plan_io,data_po,data_make,data_int_ord,data_int_req],axis=0)
...
for param in params:
    ...
    RACE_PATH_TAG = r'E:\RACE\RACE_GLOBAL'
    tags = pd.read_excel(RACE_PATH_TAG + '\\' + 'MCID_Tagging _Master.xlsx', engine='openpyxl')
    ...
    mcid_tagging_path = r"E:\_Projects\preferred_offer\snd\preferred_offer.csv"
