...
def main():
    User_Input = int(input('1) ia_consolidate | 2) order_metric | 3) EXIT : '))
...
def ia_consolidate():
    ...
    OCP_order_scheduling = OCP_order_scheduling[OCP_order_scheduling['Region'] == 'NAMER']
    ...
    OCP_order_scheduling.to_csv(r'E:\_Projects\EUMOCP_Consolidate\input\ia_bundle_namer.csv', index = False)
...
def order_metric():
    ...
    OCP_IA = pd.read_csv(r'E:\_Projects\EUMOCP_Consolidate\input\ia_bundle_namer.csv', encoding='utf-8')
   ...
   #Scheduling Date Vs IO Booked Date
    ...
    OCP_IA['Actual LT'] = (pd.to_datetime(OCP_IA['Actual Ship Date']) - pd.to_datetime(OCP_IA['Booked Dt'])).dt.days
    
    # Create conditions and choices for ageing buckets
    con1 = OCP_IA['Actual LT'] < 5 #Less 10
    con2 = OCP_IA['Actual LT'] <= 10
    
    OCP_IA['Actual LT Buket'] = np.where(con1,'00-05 days', \
                                 np.where(con2,'05-10 days','10+ days'))
    
    OCP_IA['Actual LT Buket'] = np.where((OCP_IA['Actual LT'].isna()), '', OCP_IA['Actual LT Buket'])
    ...
    OCP_IA['Pref Actual LT Buket'] = np.where((OCP_IA['Actual LT'] < OCP_IA['Unit LT']), 'On-Time', 'Late')
    OCP_IA['Pref Actual LT Buket'] = np.where((OCP_IA['Actual LT'].isna()), '', OCP_IA['Pref Actual LT Buket'])
    
    OCP_IA['Actual LT wks'] = OCP_IA['Actual LT']/7
    OCP_IA['Sys LT wks'] = OCP_IA['Unit LT']/7
    OCP_IA['Actual LT Vs Sys LT'] = OCP_IA['Actual LT wks'] - OCP_IA['Sys LT wks']
    
    # Create conditions and choices for ageing buckets
    con1 = OCP_IA['Actual LT Vs Sys LT'] < 0 #Less 10
    con2 = OCP_IA['Actual LT Vs Sys LT'] <= 1
    con3 = OCP_IA['Actual LT Vs Sys LT'] <= 2
    con4 = OCP_IA['Actual LT Vs Sys LT'] <= 3
    ...
    OCP_IA['Actual LT Vs Sys LT Bucket'] = np.where((OCP_IA['Actual LT'].isna()), '', OCP_IA['Actual LT Vs Sys LT Bucket'])
    
    
    #Upd Promise date Vs SAD%
    OCP_IA['PD vs ASD Flag'] = np.where((OCP_IA['FPD'] > OCP_IA['Actual Ship Date']), 'On-Time', 'Late') 
    ...
    #--------------------------------------------------------------------------
    #------------------------------Final Export--------------------------------
    OCP_IA.to_csv(r'E:\_Projects\EUMOCP_Consolidate\output\ia_namer_tracking.csv', index = False)
