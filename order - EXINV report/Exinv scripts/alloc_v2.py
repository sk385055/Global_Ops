...
#     file_wb = r'workbench_data_ammocp_20251202_v2.csv'
    if site =='enn':
        file='enn'
    ...
    # Key Account
    '''
    map_key = pd.read_excel(r'E:\_Projects\lookups\\Key_Account_map.xlsx', engine='openpyxl')
    map_key.drop_duplicates(['Master Customer Number'], inplace=True)
    map_key.rename(columns={'Master Customer Number':'Master Customer Number_upd', 'Key Account':'Key Account_upd'}, inplace=True)
    out = pd.merge(out, map_key[['Master Customer Number_upd','Key Account_upd']], left_on=['Master Customer Number'],right_on=['Master Customer Number_upd'], how='left')
    ...
    out['Master Customer Number'] = out['Master Customer Number'].astype(str)

    cust = cust[['Master Customer Number','Key Account']]

    cust.drop_duplicates(inplace=True)
    cust.rename(columns={'Key Account':'Key Account_upd'}, inplace=True)
    out = pd.merge(out,cust,on='Master Customer Number',how='left')
    out['Key Account'] = out['Key Account_upd']
    out['Key Account'] = out['Key Account'].apply(lambda x: "OTHER" if pd.isna(x) or x.strip() == "" else x)

    out['Key Account'] = out['Key Account'].fillna("OTHER")
    ...
    try:
        out.to_csv(r'\\wtc1501cifs.prod.local\ORGSHARE\TEAMS\ERP Shared Folder\NCR Atleos\Atleos Operations Planning\Global Ops Analytics\Metrics\Production\workbench\snd\wb_' + file + '_allocation_' + date_str + '.zip', compression={'method': 'zip', 'archive_name': 'workbench_po.csv'},index=False)
