...
if User_Input ==1:
    ...
    #OCP = pd.read_csv(r'C:\Vasanthan\Python\NCR\IND POR\Input\NCR_OCP_IND_POR_25-DEC-2023.csv',na_values=[''],keep_default_na=False,low_memory=False)
       ...
       # OCP.to_csv(r'E:\_Projects\IND POR\Output\NCR 2.csv',index=False)
        ...
        Header={"THEATRE":"THEATRE_ORGINAL","REGION":"Region","AREA":"Area","LOB REGION":"LOB Region","LOB AREA":"LOB Area","COUNTRY":"Country Name","COUNTRY_CODE":"Country Code","BILL_TO_COUNTRY":"Bill to Country","ZONE":"Zone","CUSTOMER_LOB":"Customer LOB","PRODUCT_LOB":"Product LOB","PRODUCT_RANGE":"Product Range","PARENT_MODEL":"Parent Model","CLASS":"Class","DESCRIPTION":"Description","ORGANIZATION_CODE":"Organization Code","KEY_ACCOUNT":"Key Account","MASTER_CUSTOMER":"Master Customer","ITEM_TYPE":"Item Type","DATA_SERIES":"Data Series"}
        ...
        columns_to_move = [
            'THEATRE_ORGINAL','Region', 'Area', 'LOB Region', 'LOB Area', 'Country Name', 'Country Code',
    ...
    ########## Plan ##############
       ...
       # print(len(col_name))
        ...
        for i in range(0, 6):
           ...
           if next_month_year in QTY.columns:
        ...
        filtered_df = QTY[matching_columns]
        
        # Calculate the sum of values in the selected columns and assign it to the "6M POR" column
        QTY['6M POR'] = filtered_df.sum(axis=1)
        
        QTY = QTY.filter(regex='^(?!.*Unnamed: 55$)')
        ...
        QTY = pd.concat([QTY, quarters], axis=1)
        
        
        ######################### Total ###########################3
        
        #years = set(col.split('-')[-1] for col in QTY.columns if 'Q' in col)
        years = sorted(set(col.split('-')[-1] for col in QTY.columns if 'Q' in col))
        
        # Calculate the total for each year and add new columns
        for year in years:
            year_columns = [col for col in QTY.columns if f'-{year}' in col]
            QTY[f'Total {year}'] = QTY[year_columns].sum(axis=1)
        
        print('QTY Completed')
        
        ################### Adding $ Symbole ######################
        
        QTY.columns = [col + '' for col in  QTY.columns]
    
        # Find the index of '6M POR' and slice the DataFrame
        start_index = QTY.columns.get_loc('6M POR')
        QTY = QTY.iloc[:, start_index:]
        
        #QTY.to_csv(r'E:\_Projects\IND POR\Output\QTY POR.csv',index=False)
        ...
        #years = sorted(set(col.split('-')[-1] for col in ASP.columns if 'Q' in col))
        years = sorted(set(int(col.split('-')[-1]) for col in ASP.columns if 'Q' in col))
        ...
        POR = pd.concat([OCP,QTY,ASP],axis=1)
        ...
        POR.loc[POR['THEATRE_ORGINAL'].notnull(),'Theatre'] = POR['THEATRE_ORGINAL']
        
        POR.loc[POR['LOB Region'].str.contains('NAMER'),'Theatre']='NAMER'
        
        POR.drop(columns='THEATRE_ORGINAL', inplace= True)
    ...
    # Jose written to remove Theatre
    #NCR_OCP = NCR_OCP.drop(columns=['THEATRE'])
    ...
    for i in range(20, count ,1): #Jose Comment: Refer POR Columns 
       #print(col_name[i])
       try:
         col_name[i]  = pd.to_datetime(col_name[i] , format='mixed').strftime('%b-%y')
       except: 
