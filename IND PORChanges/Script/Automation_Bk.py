...
User_Input = int(input('1) POR Report | 2) Ex-Inventory Report :'))
...
if User_Input ==1:
    ...
    #OCP = pd.read_csv(r'C:\Vasanthan\Python\NCR\IND POR\Input\NCR_OCP_IND_POR_25-DEC-2023.csv',na_values=[''],keep_default_na=False,low_memory=False)
       ...
       # OCP.to_csv(r'C:\Vasanthan\Python\NCR\IND POR\Output\NCR 1.csv',index=False)
        ...
        ################################################### ASP/MCC update ##############################
        #print('ASP')
        ASP = pd.read_csv(r'E:\_Projects\IND POR\Input\ASP.csv', na_values=[''],keep_default_na=False)
        
        ASP.columns = ASP.columns.str.strip()
        
        ASP = ASP[ASP['Override']=='Y']
        
        ASP.loc[ASP['KEY_ACCOUNT'].str.contains('CARDTRONICS', case=False),'ASP']=0
        ASP.loc[ASP['KEY_ACCOUNT'].str.contains('ATMAAS', case=False),'ASP']=0
        
        OCP['Concat']=OCP['PARENT_MODEL']+OCP['COUNTRY_CODE']+OCP['KEY_ACCOUNT']
        
        ASP['Concat'] = ASP['PARENT_MODEL']+ASP['COUNTRY_CODE']+ASP['KEY_ACCOUNT']
        
        ASP = ASP.rename(columns=lambda x: x.replace('ASP', 'ASP_x'))
        ASP = ASP[['Concat','ASP_x','Override']]
        
        OCP = pd.merge(OCP, ASP, on='Concat',how='left')
        
        OCP.loc[OCP['Override']=='Y','ASP']=OCP['ASP_x']
        OCP.loc[((OCP['ASP'].isnull())&(OCP['ASP_x'].notnull())),'ASP']=OCP['ASP_x']
        #OCP.to_csv(r'C:\Vasanthan\POR\Asp update.csv',index=False)
        
        OCP = OCP.filter(regex='^(?!.*Concat$)')
        OCP = OCP.filter(regex='^(?!.*ASP_x$)')
